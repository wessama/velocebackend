    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bookings.update", [$booking->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('asset') ? 'is-invalid' : '' }}" name="asset_id" id="asset_id" required>
                    @foreach($assets as $id => $entry)
                        <option value="{{ $id }}" {{ (old('asset_id') ? old('asset_id') : $booking->asset->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('asset'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('pickup_date') ? 'is-invalid' : '' }}" type="text" name="pickup_date" id="pickup_date" value="{{ old('pickup_date', $booking->pickup_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('dropoff_date') ? 'is-invalid' : '' }}" type="text" name="dropoff_date" id="dropoff_date" value="{{ old('dropoff_date', $booking->dropoff_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('dropoff_location') ? 'is-invalid' : '' }}" type="text" name="dropoff_location" id="dropoff_location" value="{{ old('dropoff_location', $booking->dropoff_location) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('canceled') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="canceled" id="canceled" value="1" {{ $booking->canceled || old('canceled', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('refunded') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="refunded" id="refunded" value="1" {{ $booking->refunded || old('refunded', 0) === 1 ? 'checked' : '' }}>
