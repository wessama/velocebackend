...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.update", [$asset->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="make">{{ trans('cruds.asset.fields.make') }}</label>
                <input class="form-control {{ $errors->has('make') ? 'is-invalid' : '' }}" type="text" name="make" id="make" value="{{ old('make', $asset->make) }}">
                @if($errors->has('make'))
                    <div class="invalid-feedback">
                        {{ $errors->first('make') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.make_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="model">{{ trans('cruds.asset.fields.model') }}</label>
                <input class="form-control {{ $errors->has('model') ? 'is-invalid' : '' }}" type="text" name="model" id="model" value="{{ old('model', $asset->model) }}">
                @if($errors->has('model'))
                    <div class="invalid-feedback">
                        {{ $errors->first('model') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.model_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="year">{{ trans('cruds.asset.fields.year') }}</label>
                <input class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" type="text" name="year" id="year" value="{{ old('year', $asset->year) }}">
                @if($errors->has('year'))
                    <div class="invalid-feedback">
                        {{ $errors->first('year') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{!! old('notes', $asset->notes) !!}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                <label for="price">{{ trans('cruds.asset.fields.price') }}</label>
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $asset->price) }}" step="0.01">
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                <label for="amenities">{{ trans('cruds.asset.fields.amenity') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('amenities') ? 'is-invalid' : '' }}" name="amenities[]" id="amenities" multiple>
                    @foreach($amenities as $id => $amenity)
                        <option value="{{ $id }}" {{ (in_array($id, old('amenities', [])) || $asset->amenities->contains($id)) ? 'selected' : '' }}>{{ $amenity }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.amenity_helper') }}</span>
            </div>
            <div class="form-group">
